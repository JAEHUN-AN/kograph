...
시연 리허설에서 발견했다. 컨테이너를 재시작한 직후 첫 벡터 검색이 **6.65초**다.
두 번째부터는 100~120ms로 떨어진다. INT8 모델을 지연 로딩하기 때문이다.

문제는 그 한 번이 히스토그램에 남는다는 점이다. p95가 250ms에서 1.6초로 뛰고,
대시보드를 보며 "벡터 지연은 임베딩 비용"이라고 설명하는 순간 숫자가 설명과
어긋난다. 평균이 아니라 p95를 봐도 **표본이 적을 때는 이상치 하나가 지배한다.**

`scripts/warmup.py`로 시연 전 두 축을 모두 예열한다. `/healthz` 200을 기다린
뒤 호출하는데, 대기 없이 붙으면 스택을 막 올린 직후에 `Server disconnected`로
죽어서 정작 필요한 순간에 예열이 안 된다.

startup probe를 따로 둔 이유와 같은 현상이다. 이 로딩 시간을 liveness가 보면
파드가 뜨는 도중에 죽는다.

**예열만으로는 부족했다.** 워밍업은 빠른 표본을 *더할* 뿐 이미 기록된 느린
표본을 *지우지 못한다*. 표본이 10개뿐이면 p95는 여전히 그 이상치 하나를
집는다. 실측으로 확인한 순서는 이렇다.

| 시점 | vector p95 |
|---|---|
| 콜드 호출 직후 | 9,000 ms |
| 약 5분 뒤 (5m 창에서 이탈) | **NaN** |
| 부하 주입 후 첫 스크레이프 | **230 ms** |

중간 단계가 함정이다. 일찍 예열해두면 콜드 표본은 사라지지만 트래픽이 없어
패널이 **빈 화면**이 된다. 그래서 시연 절차는 "예열 → 5분 경과 → 대시보드
직전 부하 주입" 세 박자다.

9,000ms라는 값 자체도 실제보다 크다. 버킷이 `(..., 2.5, 5.0, 10.0)`이라
6.4초 표본이 `≤10` 버킷에 들어가고 `histogram_quantile`이 5~10 구간을
선형보간한다. **꼬리 버킷이 성기면 p95는 실제보다 부풀려진다.** 정확한 꼬리
지연이 필요하면 버킷을 촘촘히 하거나 다른 집계를 써야 한다.

...
### 소스를 고쳐도 반영되지 않는다 — 프로세스가 살아 있기 때문

stdio 서버는 **클라이언트가 띄운 하위 프로세스**다. 파이썬은 모듈을 프로세스
시작 시 한 번만 읽으므로, 파일을 고쳐도 그 프로세스는 옛 코드로 계속 답한다.
핫 리로드가 없다.

여기서 시간을 크게 버릴 수 있다. 실제로 겪은 순서다.

1. 도구 출력을 고치고 테스트·린트를 통과시켰다
2. 클라이언트에서 물어봤는데 **옛 출력이 나왔다**
3. 코드가 틀렸다고 의심하며 한참 들여다봤다
4. 새 파이썬 프로세스로 같은 함수를 직접 부르니 **새 출력이 나왔다**
   → 코드는 멀쩡했고 서버 프로세스만 낡은 것이었다

**앱을 재시작해도 부족할 수 있다.** 대화 세션이 살아남으면 그 세션이 붙어 있던
프로세스도 함께 살아남는다. 확실한 방법은 **새 대화창을 여는 것**이다.

진단 순서를 고정해두면 3번에서 헤매지 않는다.

```python
# 새 프로세스에서 직접 호출해본다. 여기서 새 출력이 나오면 코드는 맞고
# 서버 프로세스가 낡은 것이다.
from your_pkg.server import your_tool
print(your_tool(...))
```

**실행 중인 코드가 언제 것인지 알 수 있게 만들어두면 더 낫다.** 정적 버전은
편집해도 그대로라 쓸모가 없으니 소스 수정 시각을 함께 남긴다.
...
- [ ] 그 클라이언트가 **최신 코드에 붙어 있는지** 확인했다
      (stdio 서버는 핫 리로드되지 않는다. 앱 재시작으로 부족하면 새 대화창)

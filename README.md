...
환각이 없고 결정론적이다. LLM 경로(`graph/extract.py`)는 설계 대안의 기록으로
남긴다 — 크레딧을 추가하지 않기로 해 실행 계획은 없다. 이 선택이 옳았는지는
추측이 아니라 측정으로 확인했다(수동 라벨 273건, 정밀도 99.6%).
...
**실행하지 않기로 확정한 것** — LLM 관계 추출(`graph/extract.py`). API 크레딧을
추가하지 않기로 결정했다. 고정 양식 공시에서 규칙 파서가 정밀도 99.6%
([005](notes/005-parser-precision.md))로 충분하다는 것이 측정으로 확인됐고,
미파싱 99건은 아래 '알려진 한계'에 그래프 공백으로 명시한다. 코드는 설계
대안의 기록으로 남긴다.
